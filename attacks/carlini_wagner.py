...
class CarliniWagnerL2Attack(Attack, LabelMixin):
    ...
    def perturb(self, x, text=None, y=None, length=None, batch_size = 192, text_loss=None, length_loss=None):
        x, y = self._verify_and_process_inputs(x, text, y)

        # Initialization
        if y is None:
            y = self._get_predicted_label(x, text)
        x = replicate_input(x)
        batch_size = len(x)
