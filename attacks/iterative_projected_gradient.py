...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn, text=None,
                      delta_init=None, minimize=False, ord=np.inf,
                      clip_min=0.0, clip_max=1.0,
                      l1_sparsity=None, length=None, text_loss=None, length_loss=None,
                      batch_size=192, CTC = True):
    """
    ...
    for ii in range(nb_iter):  
        if text is not None:
            if CTC:
                outputs = predict(xvar + delta, text)
                # print(outputs.size(1))
                preds_size = torch.IntTensor([outputs.size(1)] * batch_size) # add, 192 is the default batch_size
           
                outputs = outputs.log_softmax(2).permute(1, 0, 2) # add
                # yvar = yvar.log_softmax(2).permute(1, 0, 2) # add
                if text_loss is not None:
                    loss = loss_fn(outputs, text_loss, preds_size, length_loss) # change
                else:
                    loss = loss_fn(outputs, text, preds_size, length) # change
            else:
                outputs = predict(xvar + delta, text, is_train=False)
                if text_loss is not None:
                    outputs = outputs[:, :text_loss.shape[1] - 1, :]
                    target = text_loss[:, 1:]  # without [GO] Symbol
                    loss = loss_fn(outputs.contiguous().view(-1, outputs.shape[-1]), target.contiguous().view(-1))
                else:
                    target = text[:, 1:]  # without [GO] Symbol
                    loss = loss_fn(outputs.view(-1, outputs.shape[-1]), target.contiguous().view(-1))
        else:
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, text=None, y=None, length=None, batch_size = 192, text_loss=None, length_loss=None, CTC=True):
        ...
        rval = perturb_iterative(
            ...
            loss_fn=self.loss_fn, text=text, minimize=self.targeted,
            ord=self.ord, clip_min=self.clip_min,
            clip_max=self.clip_max, delta_init=delta,
            l1_sparsity=self.l1_sparsity, length=length, text_loss=text_loss,
            length_loss=length_loss, batch_size = batch_size, CTC = CTC
        )
